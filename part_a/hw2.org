...
fun score_challenge (card_list, goal) =
    let val aces_high_sum = sum_cards card_list
        ...
        val ideal_aces_low = if sum_goal_diff > 0 then sum_goal_diff * 4 div 10 else 0
        val num_aces_low = if ideal_aces_low > ace_count then ace_count else ideal_aces_low
        val preliminary = if aces_high_sum <= goal then goal - aces_high_sum
                          else if (aces_high_sum - num_aces_low * 10) < goal
                          then goal - (aces_high_sum - num_aces_low * 10)
                          else ((aces_high_sum - num_aces_low * 10) - goal) * 3
    in
        if all_same_color card_list
        then preliminary div 2
        else preliminary
