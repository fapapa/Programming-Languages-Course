...
fun score_challenge (card_list, goal) =
    let fun ace_counter (cards, acc) =
            case cards of
                [] => acc
              | (_, Ace)::cs => ace_counter (cs, acc + 1)
              | _::cs => ace_counter (cs, acc)
        val aces_high_sum = sum_cards card_list
        val ace_count = ace_counter (card_list, 0)
        val ideal_aces = ((aces_high_sum - goal) * 4) div 10
        val num_aces_low = if ideal_aces > ace_count then ace_count else ideal_aces
        val preliminary = if aces_high_sum > goal andalso
                             ace_count = 0
                          then (aces_high_sum - goal) * 3
                          else if aces_high_sum > goal andalso
                                  ace_count >= ideal_aces
                          then aces_high_sum - ideal_aces * 10
                          else aces_high_sum - ace_count * 10
    in
        if all_same_color card_list
        then preliminary div 2
        else preliminary
...
val test10 = score_challenge ([(Hearts, Num 2),(Clubs, Num 4)],10) = 4
      #+end_src
