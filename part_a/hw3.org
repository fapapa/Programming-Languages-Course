...
fun all_answers f xs =
    let fun list_lengths_equal (l1, l2) = (length l1 = length l2)
        val answers = List.foldl (fn (x, mem) => case f x of
                                                     NONE => mem
                                                   | SOME v => mem @ [v]) [] xs
    in if list_lengths_equal (xs, answers) then SOME answers else NONE end
