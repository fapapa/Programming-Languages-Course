...
fun count_wild_and_variable_lengths pattern =
    let val sum = ref 0
        val _ = g (fn () => let val _ = sum := (!sum) + 1 in !sum end)
                  (fn x => let val _ = sum := (!sum) + String.size x in !sum end)
                  pattern
    in
        !sum
    end
...
fun count_some_var (s, p) =
    let val matches = ref 0
        val _ = g (fn () => 0)
                  (fn str => if str = s
                             then let val _ = matches := (!matches) + 1 in !matches end
                             else !matches)
                  p
    in
        !matches
...
val first = check_pat (Variable("x")) = true
val second = check_pat (TupleP [Variable("x"), Variable("x")]) = false
    #+end_src

    #+RESULTS:
    #+begin_example
    stdIn:403.13-406.80 Error: right-hand-side of clause does not agree with function result type [tycon mismatch]
      ...
      in declaration:
        strings =
          (fn pat =>
                (case pat
                of Variable x => x :: nil
                 | TupleP ps => ((List.foldl (fn <pat> => <exp>)) nil) ps))
    stdIn:414.9-414.29 Error: operator and operand do not agree [tycon mismatch]
